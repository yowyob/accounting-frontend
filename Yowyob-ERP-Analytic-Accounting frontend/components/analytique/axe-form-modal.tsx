...
import { AxeAnalytique, TypeAxe, UniteOeuvre } from "@/lib/mock-data";
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-border sticky top-0 bg-card z-10">
                    <div>
                        <h2 className="text-base font-bold text-foreground">
                            {isEdit ? "Modifier l'axe d'analyse" : isSubAxe ? "Nouveau sous-axe" : "Nouvel axe d'analyse"}
                        </h2>
                        {isSubAxe && (
                            <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                                <GitMerge className="h-3 w-3" />
                                Sous-axe de : {allAxes.find((a) => a.id === form.parentId)?.libelle ?? form.parentId}
                            </p>
                        )}
                    </div>
